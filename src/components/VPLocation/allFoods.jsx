...
const AllFoods = () => {
    ...
    // ✅ FIX: useMemo must be OUTSIDE JSX
    const duplicatedFoods = useMemo(() => [...foods, ...foods], [foods]);
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            const totalWidth = cardSize * foods.length;
            const normalizedLeft = scrollLeft % totalWidth;
            const index = Math.round(normalizedLeft / cardSize);
    ...
    useEffect(() => {
        ...
        let rafId;
        const cardSize = 280 + 24;
        const singleListWidth = cardSize * foods.length;
        const speed = 0.6;

        const step = () => {
            if (!isPaused) {
                el.scrollLeft += speed;
                if (el.scrollLeft >= singleListWidth) {
                    el.scrollLeft -= singleListWidth;
                }
            }
